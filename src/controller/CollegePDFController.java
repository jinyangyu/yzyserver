...
public class CollegePDFController extends Controller {
	...
	public void createPDF() {
		...
		// }
		//
		// List<UserInfoModel> users = UserInfoModel.dao.find("select * from userinfo
		// where clientSession = ?",
		// clientSession);
		...
		// "select * from orders_yzy where out_trade_no = ? and type = 1 and openid = ?
		// and score=? and subject = ?",
		...
		YzyOrderModel order = new YzyOrderModel();
		...
		if (order.getPdf() != null && !"".equals(order.getPdf())) {
			...
			PDFUtil pdfUtil = new PDFUtil(colleges, score, isWen,"于金洋");
			pdfUrl = pdfUtil.createPDF();
			if ("".equals(pdfUrl)) {
		...
		renderJson(new Result(ResultCode.SUCCESS, "create success", new PdfResult(pdfUrl)));
	}
